...
void putSVGNode(FILE *file, double x, int y, char *descricao, char *posicao) {
    ...
    fprintf(file, "<text x=\"%lf\" y=\"%lf\" fill=\"white\" font-family=\"Arial\" font-size=\"%d\" dominant-baseline=\"middle\" "
                  "text-anchor=\"middle\" style=\"stroke:#000;stroke-width:1.8\">%s</text>\n", 
...
static double _generateTreeSVG(Tree arvore, FILE *file, PosicNo node, int height, double xMin, double *x, void (*describe)(Element, char*, char*)) {
    ...
    *x = (xNe + xSw) / 2;
    ...
    svg_imprimir_linha(file, *x, y, xNe, yChild - TREE_NODE_RADIUS,"black");
...
void consulta_dmprbt(char t[32], char *sfx, char *diretorioSaida, Tree arvoreQuadra, Tree arvoreHidrante, Tree arvoreSemaforo, Tree arvoreRadioBase){
    char * arqsvg3 = NULL;
    FILE *svg3;
    double x;
